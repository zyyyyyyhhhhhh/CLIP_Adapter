...
class CLIPVAD(nn.Module):
    ...
    def forward(self, visual, padding_mask, text, lengths):#这里的test就是label列表
        # print(text)
        ...
        # print(f'logits1.shape is {logits1.shape}')

        text_features_ori = self.encode_textprompt(text)
        # print(f'text_features_shape is {text_features_ori.shape}')
        ...
        # print(f'visual_attn.former_shape is {visual_attn.shape}')
        ...
        # print(logits1.shape)
        # print(f'logits2.shape is {logits2.shape}')
...
class PromptLearner(nn.Module):
    def __init__(self, classnames, clip_model, n_ctx=4, dtype=torch.float32, device='cuda'):
        ...
        for i, name in enumerate(classnames):
            if name == 'normal':
                prompt = f"This is a normal  video  of {name}"
            else:
                prompt = f"This is a abnormal video  of {name}"
...
#         self.ctx = nn.Parameter(torch.empty(self.n_cls, n_ctx, self.ctx_dim, dtype=dtype))  # 每个类别一个learnable prompt
#         nn.init.normal_(self.ctx, std=0.02)
#         prompt_prefix = " ".join(["X"] * n_ctx)
#         # prompts = []
#
#         classnames = [name.replace("_", " ") for name in classnames]
#         self.prompts = [f"This is a video of {prompt_prefix} {name}." for name in classnames]
#         self.tokenized_prompts = torch.cat([clip.tokenize(p) for p in self.prompts]).long().to(device)
#
#         with torch.no_grad():
#             embedding = clip_model.token_embedding(self.tokenized_prompts).type(dtype).to(clip_model.dtype)
#
#         self.register_buffer("token_prefix", embedding[:, :1, :])  # SOS
#         self.register_buffer("token_suffix", embedding[:, 1 + n_ctx :, :])  # CLS, EOS
#
...
#
#     def forward(self):
#         ctx = self.ctx  # 每个类别一个learnable prompt
#         print('-----------------------------------------------------------------')
...
#         print('-----------------------------------------------------------------')
#         print(f'prompts_shape:{prompts.shape}')
#         return prompts.to(self.ctx.device)
