...
from model_test3 import CLIPVAD
...
def test(model, testdataloader, maxlen, prompt_text, gt, gtsegments, gtlabels, device, best_mAP):
    model.to(device)
    model.eval()
    ...
    with torch.no_grad():
        for i, item in enumerate(testdataloader):
            ...
            # print(logits1.shape)
            # print(f'This is logits1:{logits1[1,:3]}')
            # print(logits2.shape)
            # print(f'This is logits2:{logits2[:3]}')
            logits1 = logits1.reshape(logits1.shape[0] * logits1.shape[1], logits1.shape[2])
            logits2 = logits2.reshape(logits2.shape[0] * logits2.shape[1], logits2.shape[2])
            # print(f'This is logits2_reshape:{logits2[:5]}')
    ...
    # print("element_logits2_stack:", element_logits2_stack)
    # print("gtsegments:", gtsegments)
    ...
    if best_mAP < averageMAP:
        best_mAP = averageMAP

    return ROC1, AP1, averageMAP, best_mAP, map_


if __name__ == '__main__':
    device = "cuda" if torch.cuda.is_available() else "cpu"
    args = test3_option.parser.parse_args()
    ...
    test(model, testdataloader, args.visual_length, prompt_text, gt, gtsegments, gtlabels, device, best_mAP)