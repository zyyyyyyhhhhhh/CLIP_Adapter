...
writer = SummaryWriter(log_dir='logs/test1')
    ...
    for e in range(args.max_epoch):
        ...
        for i in range(min(len(normal_loader), len(anomaly_loader))):
            ...
            if step % 1280 == 0 and step != 0:
                ...
                _map = []
                AUC, AP, mAP, _best_mAP, map_ = test(model, testloader, args.visual_length, prompt_text, gt,
                                                    gtsegments, gtlabels, device, best_mAP)
        ...
        torch.save(model.state_dict(), '../model_test1/model_cur.pth')
        checkpoint = torch.load(args.checkpoint_path)
        model.load_state_dict(checkpoint['model_state_dict'])
...
if __name__ == '__main__':
    ...
    args = test1_option.parser.parse_args()
...
#                 # print('epoch: ', e + 1, '| step: ', step, '| loss1: ', loss_total1 / (i + 1))
#                 _map = []
#                 AUC, AP, mAP, best_mAP, map_ = test(model, testloader, args.visual_length, prompt_text, gt,
#                 gtsegments, gtlabels, device, best_mAP)
#                 AP = AUC
#
#                 if AP > ap_best:
...
#         scheduler.step()
#         # if AP > ap_best:
#         print(f'Best AUC is {epoch_best_AUC}')
#         print(f'Best mAP is {epoch_best_AP}')
#
#         torch.save(model.state_dict(), '../model_test1/model_cur.pth')
#         checkpoint = torch.load(args.checkpoint_path)
#         model.load_state_dict(checkpoint['model_state_dict'])
